"""
Core puzzle logic with full integration support.
"""
...
from puzzle.config import Layout, Animation
from puzzle.models import Tile, PuzzleState
from puzzle.solver import is_solvable, get_hint, GOAL_POSITIONS
...
class NPuzzle:
    ...
    Standalone:
        puzzle = NPuzzle(standalone=True)
        puzzle.run()
    ...
    def init(self, screen: pygame.Surface, clock: pygame.time.Clock):
        """Initialize with external screen and clock."""
        self.screen = screen
        self.clock = clock
    ...
    def set_callback(self, name: str, fn: Callable):
        """Register callback: on_solve, on_move, on_reset."""
    ...
    def _generate_puzzle(self):
        """Create new random solvable puzzle."""
    ...
    def _to_grid(self) -> List[List[int]]:
        """Convert tiles to 2D grid for solver."""
    ...
    def _try_move(self, tile: Tile) -> bool:
        """Attempt to slide tile into empty space."""
        ...
        self.renderer.spawn_sparkles(
            tx + Layout.TILE_SIZE / 2,
            ty + Layout.TILE_SIZE / 2, 5
        )
    ...
    def _is_tile_correct(self, tile: Tile) -> bool:
        """Check if tile is in goal position."""
        if tile.is_empty:
            return True
        ...
        if self.state == PuzzleState.ANIMATING:
            ...
            for tile in self.tiles:
                if tile.is_moving:
                    ...
                    if abs(dx) < Animation.TILE_SNAP_DIST and abs(dy) < Animation.TILE_SNAP_DIST:
                        tile.snap_to_target()
                    else:
    ...
    def handle_event(self, event: pygame.event.Event):
        ...
        # Solved state — click anywhere to continue
    ...
    def is_complete(self) -> bool:
        """Check if puzzle was solved (for integration)."""
        return self.state == PuzzleState.SOLVED